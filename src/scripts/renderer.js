...
require("dns").resolve("www.google.com", function (err) {
  if (err) {
    ...
    checkNewAppVersion(); //commented out for now.  ~sanjay
    ...
    if (!duplicate) {
      if (contactPersonStatus === "Yes") {
        if (!existingContactPersonStatus) {
          ...
          document.getElementById("para-save-contributor-status").innerHTML =
            "<span style='color: red;'>Contact person is already added below. Per SPARC requirements, only one contact person is allowed for a dataset.</span>";
        }
      } else {
        var row = (table.insertRow(rowIndex).outerHTML =
          "<tr  id='row-current-name" +
...
const curateBFAccountLoad = document.getElementById(
  "div-bf-account-load-progress-curate"
);
const curateBFAccountLoadStatus = document.getElementById(
  "para-select-account-status-curate"
);
const curateBFAccountDetails = document.getElementById(
  "para-account-details-curate"
);
...
curateDatasetDropdown.addEventListener("change", function () {
  var curateSelectedbfdataset =
    curateDatasetDropdown.options[curateDatasetDropdown.selectedIndex].text;
...
curateBFaccountList.addEventListener("change", function () {
  ...
  var curateSelectedbfaccount =
    curateBFaccountList.options[curateBFaccountList.selectedIndex].text;
      ...
      client.invoke("api_bf_default_account_load", (error, result) => {
        if (error) {
          ...
          if (result.length > 0) {
            ...
            $("#bfallaccountlist option[value=" + myitemselect + "]").attr(
              "selected",
              "selected"
            );
            curateShowAccountDetails(curateBFaccountList);
  ...
  client.invoke(
    ...
    (error, res) => {
      if (error) {
        log.error(error);
        console.error(error);
        curateBFAccountDetails.innerHTML =
...
function showBFAddAccountBootbox() {
  bootbox.confirm({
    title:
      "Adding Blackfynn account<br><span style='font-size:12px'>Please enter your Blackfynn API key and name below:</span>",
    message:
  ...
  if (slashCount === 1) {
    ...
    for (var i = 0; i < folderArray.length; i++) {
      ...
      if (duplicate) {
        ...
        currentLocation["folders"][baseName] = {
          type: "local",
          folders: {},
          files: {},
          action: ["new"],
      ...
      if (slashCount === 1) {
        if (duplicate) {
          ...
          if (
            ...
            bootbox.alert({
              message:
                "Invalid file(s). Only SPARC metadata files are allowed in the high-level dataset folder.<br> <ul><li>dataset_description (.xslx/.csv/.json)</li><li>submission (.xslx/.csv/.json)</li><li>subjects (.xslx/.csv/.json)</li><li>samples (.xslx/.csv/.json)</li><li>CHANGES.txt</li><li>README.txt</li></ul>",
              centerVertical: true,
            });
          } else {
            myPath["files"][itemName] = {
              path: itemPath,
              description: "",
              "additional-metadata": "",
              type: "local",
  ...
  (event, filepath) => {
    if (filepath.length > 0) {
      if (filepath != null) {
        ...
        document.getElementById(
          "div-confirm-destination-locally"
        ).style.display = "flex";
      ...
      document.getElementById("div-confirm-destination-locally").style.display =
        "none";
